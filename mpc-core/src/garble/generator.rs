...
use crate::garble::FullGarbledCircuit;
...
pub fn garble<C: BlockCipher<BlockSize = U16> + BlockEncrypt>(
    ...
    let mut table: Vec<[Block; 2]> = Vec::with_capacity(circ.desc.nand);
    // Every wire label pair for the circuit
    let mut labels: Vec<Option<[Block; 2]>> = vec![None; circ.desc.nwires];
    ...
    Ok(FullGarbledCircuit {
        circ,
        wire_labels: labels.into_iter().map(|w| w.unwrap()).collect(),
...
mod tests {
    ...
    fn test_uninitialized_label() {
        ...
        let circ = &AES_128_REVERSE;

        let (input_labels, delta) = generate_labels(&mut rng, None, circ.desc.ninput_wires - 1);
