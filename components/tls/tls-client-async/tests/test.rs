...
// Expect there to be no error when server takes too long to close the socket
#[rstest]
#[tokio::test]
async fn test_ok_timeout(set_up_tls: impl Future<Output = TlsFixture>) {
    let TlsFixture {
        mut client_tls_conn,
        closed_tls_task,
    } = set_up_tls.await;
    ...
    client_tls_conn
        .write_all(&pad("must_time_out_when_closing".to_string()))
        .await
        .unwrap();
    client_tls_conn.flush().await.unwrap();
