...
>(
    ...
    let mut must_time_out_when_closing = false;
    ...
    loop {
        ...
        if conn.read_exact(&mut read_buf).await.is_err() {
            ...
            // shutdown.
            if must_time_out_when_closing {
                // sleep long enough to trigger a timeout on the client side
                // (client's default timeout value is 1000 ms)
        ...
        match s {
            "must_time_out_when_closing" => {
                // don't close the socket immediately thus triggering a time out on the client side
                must_time_out_when_closing = true;
            }
            "send_close_notify" => {
