...
use crate::{
    config::{VerifierConfig, VerifierMode},
...
enum State {
    ...
    Accepted {
        verifier: Verifier<state::CommitAccepted>,
        handle: SessionHandle,
    },
    Complete,
    Error,
    ...
    pub async fn setup(&mut self) -> Result<Option<String>> {
        ...
        match request.protocol() {
            ...
            TlsCommitProtocolConfig::Proxy(_) => {
                // No limit validation needed for proxy mode.
            }
            _ => {
        ...
        let proxy_server_name = verifier
            .is_proxy_setup_required()
            .map(|name| name.as_str().to_string());

        self.state = State::Accepted { verifier, handle };

        Ok(proxy_server_name)
    }

    /// Sets the proxy sockets for proxy mode.
    ///
    /// Must be called after `setup()` returns `Some(server_name)` and before
    /// `verify()`.
    pub fn set_proxy_sockets(&mut self, proxy_io: impl Io, server_io: impl Io) -> Result<()> {
        let State::Accepted {
            ref mut verifier, ..
        } = self.state
        else {
        ...
        verifier.set_proxy_sockets(proxy_io, server_io);
        Ok(())
    }

    /// Verifies the connection and finalizes the protocol.
    pub async fn verify(&mut self) -> Result<VerifierOutput> {
        let State::Accepted { verifier, handle } = self.state.take() else {
            return Err(SdkError::invalid_state("verifier is not in accepted state"));
        };
