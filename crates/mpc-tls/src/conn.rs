...
impl Conn {
    ...
    /// Processes a fully-parsed TLS message received while the connection is in
    /// the online (post-handshake) phase.
    ///
    ...
    pub(crate) async fn process_online(&mut self, msg: Message) -> Result<(), Error> {
        ...
        if !self.io.is_tls13() && msg.is_handshake_type(HandshakeType::HelloRequest) {
        ...
        match msg.payload {
            MessagePayload::ApplicationData(payload) => self.io.take_received_plaintext(payload),
            ...
            MessagePayload::Handshake(HandshakeMessagePayload {
                ...
                // A key update must not be interleaved with a fragmented
                // handshake message, and the client does not support key
                // updates.
                self.check_aligned_handshake().await?;
                self.send_fatal_alert(AlertDescription::InternalError)
                    .await?;
