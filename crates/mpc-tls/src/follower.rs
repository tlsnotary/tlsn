...
use tlsn_core::{
    connection::{HandshakeData, HandshakeDataV1_2, TlsVersion},
...
impl MpcTlsFollower {
    ...
    pub async fn run(mut self) -> Result<(Context, TlsTranscript), MpcTlsError> {
        ...
        let cf = transcript.sent().first().ok_or(MpcTlsError::other(
            "client finished was not first sent message",
        ))?;
        let sf = transcript.recv().first().ok_or(MpcTlsError::other(
            "server finished was not first received message",
        ))?;

        if cf
            .plaintext
            .as_ref()
            .ok_or(MpcTlsError::other("client finished plaintext is missing"))?
            .as_slice()
            != cf_vd
        {
            return Err(MpcTlsError::other("client finished inconsistent"));
        }

        if sf
            .plaintext
            .as_ref()
            .ok_or(MpcTlsError::other("server finished plaintext is missing"))?
            .as_slice()
            != sf_vd
        {
            return Err(MpcTlsError::other("server finished inconsistent"));
        }
