## Simple Proxy Verifier: Verifying Data from an API in Rust

This example demonstrates how to use TLSNotary in proxy mode, where the Verifier forwards traffic between the Prover and the server instead of using MPC.
...
```
Next, run the proxy example with:
```shell
SERVER_PORT=4000 cargo run --release --example proxy
```
To view more detailed debug information, use the following command:
```
RUST_LOG=debug,yamux=info,uid_mux=info SERVER_PORT=4000 cargo run --release --example proxy
```
