...
async fn main() {
    ...
    // Generate ENS attestation if environment variable is set.
    if let (Ok(rpc_url), Ok(sk)) = (env::var("RPC_URL"), env::var("EAS_SK")) {
        let signer = sk
            .parse::<LocalWallet>()
        ...
        let verifying_contract =
            Address::from_str(EAS_ADDRESS).expect("Failed to parse EAS address");
