...
use tlsn_mux::Config as MuxConfig;
...
// THROWAWAY stress test: full MPC-TLS session over a deliberately
// constrained mux config (tight stream budget, tiny frames) to stress the
// SYN-opener in a TLS context. Not for upstream.
const MAX_SENT_DATA: usize = 1 << 11;
const MAX_RECV_DATA: usize = 1 << 11;

fn stressed_mux_config() -> MuxConfig {
    let mut cfg = MuxConfig::default();
    // Over budget on purpose (demand exceeds 32): overbudgeting must
    // surface the expected mux error and fail fast, without hanging.
    cfg.set_max_num_streams(32);
    cfg.set_split_send_size(512);
...
crate::test!(
    "mux_tls_stress",
    prover_mux_tls_stress,
    verifier_mux_tls_stress
);

async fn prover_mux_tls_stress(provider: &IoProvider) {
    let io = provider.provide_proto_io().await.unwrap();
    let mut session = Session::with_config(io, stressed_mux_config());
    let prover = session
...
async fn finish_prover(mut prover: Prover<state::Committed>, handle: tlsn::SessionHandle) {
...
async fn verifier_mux_tls_stress(provider: &IoProvider) {
    let io = provider.provide_proto_io().await.unwrap();
    let mut session = Session::with_config(io, stressed_mux_config());
