...
crate::test!("proxy", prover, verifier);
...
async fn prover(provider: &IoProvider) {
    ...
    // In proxy mode, connect TLS through the proxy channel instead of directly
    // to the server.
    let (tls_connection, prover_fut) = prover
        .connect(
            TlsClientConfig::builder()
                ...
                .build()
                .unwrap(),
            provider.provide_proxy_io().await.unwrap(),
        )
        .unwrap();
...
async fn verifier(provider: &IoProvider) {
    ...
    // In proxy mode, set up the proxy sockets for TLS traffic forwarding.
