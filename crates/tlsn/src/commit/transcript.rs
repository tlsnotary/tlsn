...
impl TranscriptRefs {
    ...
    ///
    /// Queries that cannot or only partially be satisfied will return `None`.
    ///
    /// # Arguments
    ///
    /// * `direction` - The direction of the transcript.
...
/// information about possibly non-contigious memory locations.
#[derive(Debug, Clone)]
struct RefStorage {
    index: RangeSet<usize>,
...
impl RefStorage {
    fn new(max_len: usize) -> Self {
        Self {
            ...
            offset: Vec::default(),
            // Internal storage is bit-adressed
            max_len: 8 * max_len,
        }
    }
    ...
    fn add(&mut self, index: &Range<usize>, data: Vector<U8>) {
        ...
        self.offset[bit_index.clone()].fill(offset);
    }

    fn mark_decoded(&mut self, index: &RangeSet<usize>) {
        let bit_index = to_bit_index(index);
        assert!(
            bit_index.is_subset(&self.index),
            "Only transcript refs which have been added can be marked as decoded"
        );
        assert!(
            bit_index.is_disjoint(&self.decoded),
            "Some parts of the provided index are already marked as decoded"
        );
