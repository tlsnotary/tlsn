...
pub(crate) struct TlsParser {
    ...
    handshake: Option<(TlsVersion, HandshakeData)>,
}
...
impl TlsParser {
    ...
    /// Parses and caches the handshake data from the raw TLS records.
    pub(crate) fn parse_handshake(&mut self) -> Result<HandshakeData, TlsParserError> {
        ...
        self.handshake = Some((version, handshake.clone()));
        Ok(handshake)
    }

    /// Creates the `TlsTranscript`.
    pub(crate) fn build(&mut self) -> Result<TlsTranscript, TlsParserError> {
        ...
        let (version, handshake) = match self.handshake.take() {
            Some(cached) => cached,
            None => parse_handshake(&sent_raw_records, &recv_raw_records)?,
        };
