use crate::{
    ...
    deps::ProverZk,
    proxy::{References, VerifyDataCheck, alloc_ghash_key},
};
...
impl ProxyProver {
    pub(crate) fn new(config: &ProxyTlsConfig, vm: ProverZk, ctx: Context) -> Self {
        let prf_config = match config.network() {
            NetworkSetting::Bandwidth => PrfConfig::new(NetworkMode::Normal, MSMode::Extended),
            NetworkSetting::Latency => PrfConfig::new(NetworkMode::Reduced, MSMode::Extended),
        };
    ...
    pub(crate) fn alloc(&mut self) -> Result<(), TlsnError> {
        ...
        self.cf_vd_check.alloc(vm, &mut encrypt, prf_output.cf_vd)?;
        self.sf_vd_check.alloc(vm, &mut decrypt, prf_output.sf_vd)?;
