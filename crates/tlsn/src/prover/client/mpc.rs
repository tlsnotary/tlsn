...
use crate::{
    ...
    prover::client::{ClientHandle, Command, TlsClient, TlsOutput},
    tag::verify_tags,
};
...
impl TlsClient for MpcTlsClient {
    ...
    fn client_close(&mut self) -> Result<(), Self::Error> {
        self.sender
            ...
            .map_err(|_| TlsnError::internal().with_msg("unable to close connection clientside"))
    }

    fn server_close(&mut self) -> Result<(), Self::Error> {
        self.sender
            .send(Command::ServerClose)
            .map_err(|_| TlsnError::internal().with_msg("unable to close connection serverside"))
    }
...
impl InnerState {
    ...
    #[instrument(parent = &self.span, level = "debug", skip_all, err)]
    async fn set_decrypt(self: Box<Self>, enable: bool) -> Result<Box<Self>, TlsnError> {
        self.mpc_ctrl
