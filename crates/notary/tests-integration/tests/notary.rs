...
use notary_server::{
    ...
    JwtAuthorizationProperties, JwtClaim, LoggingProperties, NotarizationProperties,
...
const JWT_PRIVATE_KEY: &[u8] = include_bytes!("../../server/fixture/auth/jwt.key");
        ...
        authorization: AuthorizationProperties {
            ...
            mode: auth.map(|mode| match mode {
                AuthMode::Jwt => AuthorizationModeProperties::Jwt(JwtAuthorizationProperties {
                    public_key_pem_path: "../server/fixture/auth/jwt.key.pub".to_string(),
                    claims: vec![JwtClaim {
                        name: "sub".to_string(),
                        ..Default::default()
                    }],
                ...
                AuthMode::Whitelist => AuthorizationModeProperties::Whitelist(
                    "../server/fixture/auth/whitelist.csv".to_string(),
                ),
            }),
        },
        concurrency,
...
fn tcp_prover_client(notary_config: NotaryServerProperties) -> NotaryClient {
    ...
    if notary_config.authorization.enabled {
        match notary_config.authorization.mode.unwrap() {
...
#[case::tcp_with_auth_whitelist_mode({
    tcp_prover(setup_config_and_server(100, 7048, false, Some(AuthMode::Whitelist), 100).await)
})]
#[case::tcp_with_auth_jwt_mode({
    tcp_prover(setup_config_and_server(100, 7048, false, Some(AuthMode::Jwt), 100).await)
})]
#[case::tcp_without_auth({
    tcp_prover(setup_config_and_server(100, 7049, false, None, 100).await)
})]
#[awt]
#[tokio::test]
#[ignore = "expensive"]
