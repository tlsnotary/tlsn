...
pub use tlsn::verifier::VerifierConfig;
pub use tlsn_core::VerifierOutput;
...
impl Verifier<state::Committed> {
    // pub async fn verify(&mut self, config: &VerifyConfig) ->
    // Result<VerifierOutput, VerifierError> {     let config =
    // bincode::serialize(&config).unwrap();

    //     self.handle.verify(&config);

    //     let res = poll_fn(|_| {
    //         if let abi::verify::VerifyReturn::Ready(res) =
    // self.handle.finish_verify() {             Poll::Ready(res)
    //         } else {
    //             Poll::Pending
    //         }
    //     })
