...
use crate::{
    ...
    service::{axum_websocket::WebSocket, notary_service, verify_service},
};
...
pub async fn websocket_verify(
    ...
    // Wrap the websocket in WsStream so that we have AsyncRead and AsyncWrite implemented
    let stream = WsStream::new(socket.into_inner());
    match verify_service(stream, &notary_globals.notary_signing_key, &session_id).await {
        Ok(_) => {
            info!(?session_id, "Successful verification using websocket!");
        }
        Err(err) => {
            error!(?session_id, "Failed verification using websocket: {err}");
        }
    }
}
