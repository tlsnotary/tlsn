...
pub enum GarbleMessage {
    ...
    Output(Output),
    HashCommitment(mpc_core::msgs::HashCommitment),
    CommitmentOpening(mpc_core::msgs::CommitmentOpening),
...
impl From<crate::CircuitOpening> for CircuitOpening {
    fn from(opening: crate::CircuitOpening) -> Self {
        Self {
...
impl label::OutputLabelsCommitment {
    pub fn from_msg(
        ...
        if commitment.commitments.len() != output.len() * 2 {
            return Err(crate::EncodingError::InvalidLabelCommitment(
                output.id().clone(),
            ))?;
        }
