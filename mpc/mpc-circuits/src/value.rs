...
mod tests {
    ...
    fn test_value_new(
        ...
        let value = Value::new_from_msb0(value_type, string_to_boolvec(bits.as_ref())).unwrap();
    ...
    fn test_value_wrong_bit_length(#[case] value_type: ValueType, #[case] bits: impl AsRef<str>) {
        let err = Value::new_from_msb0(value_type, string_to_boolvec(bits.as_ref())).unwrap_err();
    ...
    fn test_value_to_bits(#[case] value: impl Into<Value>, #[case] expected: impl AsRef<str>) {
        let value: Value = value.into();
        assert_eq!(value.to_msb0_bits(), string_to_boolvec(expected.as_ref()));
    }
}
