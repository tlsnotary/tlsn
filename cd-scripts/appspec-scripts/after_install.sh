...
if [ $APP_NAME = "stable" ]; then
  ...
  sudo mkdir -p ~/${APP_NAME}_${TAG}/tlsn/notary-server/target/release
  ...
  # Download binary
  aws s3 cp s3://tlsn-deploy/$APP_NAME/notary-server ~/${APP_NAME}_${TAG}/tlsn/notary-server/target/release
  chmod +x ~/${APP_NAME}_${TAG}/tlsn/notary-server/target/release/notary-server
else
