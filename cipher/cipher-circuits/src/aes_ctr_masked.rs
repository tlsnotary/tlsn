...
pub fn aes_ctr_masked() -> Arc<Circuit> {
    let mut builder = CircuitBuilder::new(
        "AESCTR",
        "AES-CTR encrypts a block of text",
        "0.1.0",
        BitOrder::Msb0,
    );
    ...
    let aes_ectr = builder.add_circ(&AES_128);
    ...
    // Implicit nonce
    builder.connect(&iv[..], &aes_ectr_m[..32]);
    // Explicit nonce
    builder.connect(&nonce[..], &aes_ectr_m[32..96]);
    // Counter
    builder.connect(&ctr[..], &aes_ectr_m[96..]);
    ...
    fn test_aes_ctr_masked() {
        ...
        test_circ(
            ...
            &[
                ...
                Value::Bytes(explicit_nonce.to_vec()),
                Value::U32(ctr),
                Value::Bytes(mask_0.to_vec()),
                Value::Bytes(mask_1.to_vec()),
            ],
            &[Value::Bytes(masked_encrypted_key_block.to_vec())],
        );
    }
}
