...
Ordinarily, the output labels of a 2PC garbled circuit computation are 128-bit strings `wᵢ ∈ {0,1}¹²⁸`. But for the protocol above to work, `wᵢ` need to have algebraic structure. Fortunately, we can retrofit this requirement to any garbled-circuit scheme as follows.

Let `Wᵢⱼ' ∈ {0,1}¹²⁸` represent a bitstring label of the underlying garbled-circuit scheme. The garbler constructs its _algebraic labels_ `Wᵢⱼ ∈ 𝔽` as follows. First, it randomly selects a PRG seed `ρ`, and then generates a vector of `n` field elements `W₀ = PRG(ρ)`. The garbler also uniformly selects `Δ ∈ 𝔽`, and sets `W₁ = W₀ + (Δ, Δ, ..., Δ)`. The `W₀` and `W₁` represent the labels of the 0 bits and the 1 bits, respectively. In order to transmit the algebraic labels to the Requester, the Notary encrypts each algebraic label using the nonalgebraic label as a key. Concretely, the Notary computes `Cᵢⱼ = Enc_{Wᵢⱼ'}(Wᵢⱼ)` (using AES-GCM, since `Wᵢⱼ` is two block widths), shuffles `C` such that each `Cᵢ₀` is swapped with `Cᵢ₁` with probability 1/2, and sends `C` to the Requester.
