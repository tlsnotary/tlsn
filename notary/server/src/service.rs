...
pub async fn notary_service<T: AsyncWrite + AsyncRead + Send + Unpin + 'static>(
    ...
    let mut config_validator_builder = ProtocolConfigValidator::builder();

    if let Some(max_sent_data) = max_sent_data {
        config_validator_builder.max_sent_data(max_sent_data);
    }

    if let Some(max_recv_data) = max_recv_data {
        config_validator_builder.max_recv_data(max_recv_data);
    }

    let config_validator = config_validator_builder.build()?;
